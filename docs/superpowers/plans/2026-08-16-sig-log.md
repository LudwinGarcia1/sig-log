...
- [ ] **Step 5: Write `apps/core/navigation.py` and `templates/base.html`**

The navigation bar is **data, not markup**. Hard-coding `{% url 'customer_list' %}` in `base.html` would make this task's own tests fail with `NoReverseMatch`, because `crud_list.html` extends `base.html` and none of the module URLs exist yet. Each module task appends one line to this list instead.
...
Register it in `config/settings.py` under `TEMPLATES[0]["OPTIONS"]["context_processors"]`:
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="nav">
      <ul class="navbar-nav me-auto">
        {% for url_name, label, emphasised in nav_items %}
        <li class="nav-item">
          <a class="nav-link {% if emphasised %}fw-semibold{% endif %}" href="{% url url_name %}">{{ label }}</a>
        </li>
        {% endfor %}
      </ul>
    </div>
...
Each module registers itself at the bottom of its `urls.py`, for example in `apps/customers/urls.py`:
...
The order of `INSTALLED_APPS` and of the `include()` calls in `config/urls.py` therefore determines the navbar order — keep both in module order: Clientes, Vehículos, Operadores, Rutas, Entregas, Combustible, Mantenimiento, Reportes.
...
**Every row must carry explicit `created_at` and `updated_at` values** — add `"created_at": "2026-08-16T00:00:00Z", "updated_at": "2026-08-16T00:00:00Z"` to each `fields` object above.

It is tempting to omit them on the grounds that `auto_now_add` / `auto_now` will fill them in. They will not. `loaddata` saves with `raw=True`, and Django's `SQLInsertCompiler.pre_save_val` returns the raw attribute instead of calling `field.pre_save()` whenever `raw` is set — so the automatic values never fire and PostgreSQL rejects the insert on the NOT NULL constraint. Any future fixture in this project needs the same treatment.
...
def efficiency_report(start=None, end=None):
    ...
    for row in aggregated:
        ...
        # Tank-to-tank method. The earliest load in the window is the fill that
        # put the vehicle AT the first odometer reading — that fuel has not been
        # burned yet inside the measured span. Every later fill replaces fuel
        # that was burned covering the span, so those are the litres that count.
        # Reporting `liters` as the full sum is still correct: it is fuel bought,
        # not fuel burned.
        earliest_load = (
...
def complete_maintenance(maintenance, next_service_km=None):
    ...
    # Identity check, not truthiness: Decimal("0.00") is falsy, so `or` would
    # silently discard an explicitly-passed zero and substitute the default.
    target = (
        next_service_km
...
**Measured output of the reference run** (`--months 18 --seed 42`, under ten seconds): 120 clientes · 50 vehículos · 40 operadores · 60 rutas · **27 158 entregas cerradas** · **3 633 cargas de combustible** (72.7 per vehicle) · **565 mantenimientos** (11.3 per vehicle). The last three fall out of the generator's formulas rather than being set by hand. Use these figures, not estimates, in Tasks 21–23.

**Verified seeded pattern**, after the Task 14 coefficient correction: congested zones (METROPOLITANA, ORIENTE) run **0.6792** late across 17,586 deliveries against **0.1107** across 9,572 elsewhere; overall **0.4788**.

**Downstream results these figures produce:** classifier F1 **0.7785** (accuracy 0.7512, precision 0.6783, recall 0.9134, ROC-AUC 0.7774, CV F1 0.7789 ± 0.0040); regression MSE **787.45**, RMSE 28.06, MAE **20.93**, R² **0.2239**. Note that precision tracks the congested-zone delay rate almost exactly — a Bernoulli outcome with p = 0.679 cannot yield higher positive-class precision, which is the structural ceiling this dataset imposes and worth stating in the Unidad III document.
...
class ValidationTest(SimpleTestCase):
    ...
    def test_a_healthy_pickup_is_not_an_outlier(self):
        """8.75 km/L is BASE_EFFICIENCY["PICKUP"] at its +8% variation.

        An upper bound of 8.0 used to quarantine 40% of pickup fuel loads and
        report them as anomalies.
        """
...
# The band must be wider than every vehicle type the fleet actually contains,
# or the rule quarantines healthy data. BASE_EFFICIENCY tops out at 8.10 km/L
# for a PICKUP, and the generator varies it by up to +8%, so a sound pickup
# reaches 8.75 — an upper bound of 8.0 would reject 40% of pickup loads while
# claiming they were anomalies. Loaded trailers sit near 2.2 km/L, so 1.0 is a
# safe floor. 12.0 still catches the real anomalies, which read in the hundreds.
...
def _load_dimension(etl_run, table_name, model, natural_key, records, attributes):
    ...
    with etl_run.phase("LOAD", table_name) as counter:
        ...
        for record in records:
            ...
            if instance is None:
                # Build kwargs from the declared columns only. Transform emits
                # some fields a dimension does not store — `tax_id` on customers,
                # for one — and `model(**record)` would raise TypeError on them.
                to_create.append(model(**{
                    natural_key: key,
                    **{name: record[name] for name in attributes},
                }))
            else:
...
`base_analytics.html` reverses all six analytics URL names, so all six must resolve from this task onward or the dashboard cannot render. The five views that belong to Tasks 18 and 19 are registered now as placeholders and replaced there.

Append to `views.py`:

```python
def pending_section(request, section, title):
    """Placeholder for an analytics screen that a later task implements.

    Registered from Task 17 so base_analytics.html can reverse every URL name
    it links to. Tasks 18 and 19 replace these with the real views.
    """
    return render(request, "analytics/pending.html", {
        "section": section,
        "warehouse_empty": False,
        "pending_title": title,
    })
...
`templates/analytics/pending.html`:
...
<div class="alert alert-secondary">
  <strong>{{ pending_title }}</strong> — esta sección se habilita en una etapa posterior.
</div>
{% endblock %}
```
...
urlpatterns = [
    ...
    path("operacion/",
         partial(views.pending_section, section="operations", title="Operación"),
         name="analytics_operations"),
    path("costos/",
         partial(views.pending_section, section="costs", title="Costos"),
         name="analytics_costs"),
    path("mantenimiento/",
         partial(views.pending_section, section="alerts", title="Mantenimiento"),
         name="analytics_alerts"),
    path("prediccion/",
         partial(views.pending_section, section="predictions", title="Predicción"),
         name="analytics_predictions"),
    path("conglomerados/",
         partial(views.pending_section, section="clusters", title="Conglomerados"),
...
Tasks 18 and 19 delete the placeholder rows they supersede. When the last one goes, delete `pending_section` and `templates/analytics/pending.html` too.
...
- [ ] **Step 4: Replace the three placeholder routes in `apps/analytics/urls.py`**

Delete the `operacion/`, `costos/` and `mantenimiento/` rows that Task 17 registered with `partial(views.pending_section, …)` and put the real views in their place, keeping the same URL names and the same order:
...
Leave the `prediccion/` and `conglomerados/` placeholders alone — Task 19 owns them.
...
The `<tbody>` is intentionally empty: the JavaScript at the bottom of the page builds every row from `heatmap-data`, which keeps the arithmetic out of the template.
...
- [ ] **Step 5: Replace the last two placeholder routes in `apps/analytics/urls.py`**

Delete the `prediccion/` and `conglomerados/` rows that Task 17 registered with `partial(views.pending_section, …)` and put the real views in their place:
...
No placeholder rows remain, so also delete `pending_section` from `apps/analytics/views.py`, delete `templates/analytics/pending.html`, and drop the now-unused `from functools import partial` import.
